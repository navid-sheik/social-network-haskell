...
process :: String  -> MVar Int -> MVar [Message]->MVar String ->IO()
process  name count messages winner  = do
    c1 <- takeMVar count
    ...
    if c1 == 3 then
        ...
        putStrLn $ " -- Increasing count coin back in the box"
        let newMessage  = Message{text  = "anothitner", from = "ua", to="ua"}
        let new_messages = ms ++ [new_message]
        putMVar count $! c1 + 1
        threadDelay 5
        process  name count messages winner
...
main = do
    ...
    forkIO(process  "user1"  count box_messages winner)
    forkIO(process  "user2"  count box_messages winner)
    w <- takeMVar winner
    putStrLn $ show(w)
