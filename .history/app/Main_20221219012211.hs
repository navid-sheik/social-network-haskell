...
generateRandomDate :: IO String
generateRandomDate = do
    let random_dates = [fromGregorian 2022 1 1, fromGregorian 2023 1 3 .. fromGregorian 2023 1 1]
    let pickedDate atRandIndex messages
    return pickedDate
...
printChatHistory  message username =  
    if from message == username
        then "Me : "  ++ text message ++ " ." 
        else "" ++ from message ++ " : "  ++ text message ++ " ." 
    ...
    if c1 == 10 then
        ...
        if same_topics == True 
            then do
                ...
                let generatedDate = generateRandomDate
...
main :: IO ()
main = do
    let user1 = User {username="navid", email="tddj@gmail.com",favorite_topics=["kimmy" , "idon'tknow", "somethingelse", "blena"],  messages=[]}  
