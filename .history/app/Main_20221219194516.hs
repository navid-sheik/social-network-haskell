...
process  user count_message messages final_messages_shared users = do
...
main = do
    -- Initialize users
    let user1 = User {username="navid", email="tddj@gmail.com",favorite_topics=["cdvd" , "fvvd", "dvdv", "dv"]}  
    ...
    let messages_array = []
    count_message <- newMVar 0
    final_messages_shared <- newEmptyMVar
    messages <- newMVar messages_array

    -- Intiialise threads 
    _ <- forkIO(process  user1  count_message messages final_messages_shared array_user)
    _ <- forkIO(process  user2  count_message messages final_messages_shared array_user)
    _ <- forkIO(process  user3  count_message messages final_messages_shared array_user)
    _ <- forkIO(process  user4  count_message  messages final_messages_shared array_user)
