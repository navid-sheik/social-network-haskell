...
process :: String ->MVar String -> MVar Int -> MVar [Message] ->IO()
process winner name count messages  = do
    c1 <- takeMVar count
    putStrLn $ "The count is " ++ show(c1)
    if c1 == 3 then
         putMVar winner $ "Count is " ++ show(c1) ++ " !"
    else do
        putStrLn $ " -- Increasing count coin back in the box"
        putMVar count $! c1 + 1
        threadDelay 5
        process winner name count messages
...
main = do
    ...
    forkIO(process "user1" winner count box_messages)
    forkIO(process "user2" winner count box_messages)
    w <- takeMVar winner
    putStrLn $ w
