...
addNewMessage  message  user= user { messages = message user ++ [message] }

main :: IO ()
main = do
    let user1 = User {username="navid", email="tddj@gmail.com", messages=[]}  
    
    let new_user = addNewMessage  Message{text  = "anothitner", from = "ua", to="ua"} user1
    putStrLn(show new_user)
