...
generateRandomMessage = do 
    ...
    let random_message =  atRandIndex random_messages
...
process  user count messages winner users = do
    ...
    if c1 == 10 then
        ...
        if same_interests == True 
            then do
                random_message_user <- generateRandomMessage
                ...
                let newMessage  = Message{text  = random_message_user, from = username user, to=username random_user, date = generatedDate}
