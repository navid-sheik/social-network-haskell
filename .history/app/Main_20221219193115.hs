...
process  user count messages final_messages_shared users = do
    ...
    message_sent_sofar <- takeMVar messages
    -- Helping messages 
    -- putStrLn $ "The messages are" ++ show(message_sent_sofar)
    -- putStrLn $ "The count is " ++ show(count_message)

    -- Exit if the 100 messages are sent
    if count_message == 10 then
        putMVar final_messages_shared $ message_sent_sofar
    else do
        putStrLn $ "--------------------------------------------------"
        ...
        if same_interests == True 
            then do
                ...
                let new_messages = message_sent_sofar ++ [newMessage]
                ...
                putStrLn $ "The random user selected doesn't have any common interests"
                -- Just leave the thread variable  as before, no changes made
                putMVar messages $! message_sent_sofar
...
main = do
    ...
    -- Intiialise threads 
    forkIO(process  user1  count messages final_messages_shared array_user)
    forkIO(process  user2  count messages final_messages_shared array_user)
    forkIO(process  user3  count messages final_messages_shared array_user)
    forkIO(process  user4  count messages final_messages_shared array_user)
