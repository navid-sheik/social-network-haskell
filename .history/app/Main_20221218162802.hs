...
process  user count messages winner users = do
    ...
    if c1 == 10 then
        putMVar winner $ "Count is " ++ show(username user) ++ " !" 
    else do
        putStrLn $ " Process with name " ++ show(username user) ++ " !"
        random_user <- atRandIndex users
        putStrLn  $ "The random user selected is  " ++ show(username random_user)
        
        let newMessage  = Message{text  = "anothitner", from = username user, to="ua"}
