...
showConversationBetweenTwo all_message_users logged_user other_user  =  do
    let filtered_messages  = filter(\message_uni -> to message_uni == logged_user || from message_uni == logged_user || to message_uni == other_user || from message_uni == other_user) $ sort all_message_users 
