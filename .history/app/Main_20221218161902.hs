...
process :: User  -> MVar Int -> MVar [Message]->MVar String -> [User]->IO()
process  user count messages winner users = do
    ...
    random_user_username <- username random_user
    putStrLn  $ "The ranomd is " ++ show(random_user_username)
    if c1 == 10 then
        putMVar winner $ "Count is " ++ (username user) ++ " !" 
    else do
        putStrLn $ " Process with " ++ (username user) ++ " increasing !"
