...
process  user count messages final_messages_shared users = do
    ...
    -- Helping messages 
    -- putStrLn $ "The messages are" ++ show(message_sent_so_far)
    ...
    if count_message == 10 then
        ...
        if same_interests == True 
            then do
                ...
                putStrLn  $ "The current user " ++ show(username user)  ++ " doesn't have any interest in common (no message) " ++ show(username random_user) ++ " !"
...
main = do
    -- Initialize users
    let user1 = User {username="navid", email="tddj@gmail.com",favorite_topics=["cdvd" , "fvvd", "dvdv", "dv"],  messages=[]}  
